...
const BrandGlyph = ({ className }: { className?: string }) => (
  <svg
    ...
    className={cn("text-[hsl(38,72%,48%)]", className)}
    aria-hidden="true"
  >
    <path
...
const Brandmark = ({ stacked = false, subtitle, className, textClassName, subtitleClassName }: BrandmarkProps) => (
  <div
    className={cn(
  ...
  >
    <BrandGlyph className={stacked ? "h-16 w-auto" : "h-10 w-auto"} />
    <div className={cn(stacked ? "" : "space-y-0.5", stacked ? "" : "flex flex-col")}>
      <p className={cn("font-bold uppercase tracking-[0.2em]", stacked ? "text-sm" : "text-base", textClassName)}>
        Grace Apartments
      </p>
      {subtitle && (
        <p className={cn("text-[0.65rem] uppercase tracking-[0.45em] text-muted-foreground", subtitleClassName)}>
          {subtitle}
        </p>
      )}
    </div>
